SpeakTypesSettings = {
  ...
  alpha = { speakType = MessageModes.All, color = 'alpha' },
}
...
function getStringIcon(str)
	...
	['11'] = '?', 
	[':('] = 'ã', ['13'] = 'O', ['14'] = '?', ['15'] = '?',
	['16'] = '?', ['17'] = 'ê', ['18'] = 'ë', ['19'] = 'í', ['20'] = 'ì',
	['21'] = 'î', ['22'] = 'ï', ['23'] = 'ñ', ['24'] = 'ó', ['25'] = '~',
	['26'] = 'ô', ['27'] = '?', ['28'] = 'õ', ['29'] = 'o', ['30'] = '?',
	['31'] = '?', ['32'] = 'Y', ['33'] = 'ó', ['34'] = '∞', ['35'] = '±',
	['36'] = '2', ['37'] = '3', ['38'] = '¥', ['39'] = 'o', ['40'] =  '?',
	}
	...
	local newstr = ''
	for i = 1, #word do
		if icon_range[word[i]:sub(1, 2)] then
			newstr = newstr..icon_range[word[i]:sub(1, 2)]..' '
		else
...
function addText(text, speaktype, tabName, creatureName, guild, specialName)
  local tab = getTab(tabName)
  if tab ~= nil then
    addTabText(text, speaktype, tab, creatureName, guild, specialName)
  end
end
...
local letterWidth = {  -- New line (10) and Space (32) have width 1 because they are printed and not replaced with spacer
  ...
  [80] = 7, [81] = 8, [82] = 7, [83] = 7, [84] = 8, [85] = 8, [86] = 8, [87] = 12, [88] = 8, [89] = 8, [90] = 7, [91] = 5, [92] = 8, [93] = 5, [94] = 9, [95] = 8,
...
function getHighlightedText2(text)
  local tmpData = {}
...
function getHighlightedText3(text)
  local tmpData = {}
...
function addTabText(text, speaktype, tab, creatureName, guild, specialName)
...
--, guild, specialName
	...
	if speaktype.speakType == MessageModes.Yell or speaktype.speakType == MessageModes.Say or speaktype.speakType == MessageModes.Whisper or speaktype.speakType == MessageModes.Channel then

		if(specialName and specialName ~= "")then
			local highlightData2 = getHighlightedText2(text)
		else
			local highlightData2 = {}		
		end
		
		local highlightData2 = getHighlightedText2(text:sub(0, string.find(text, ":%s")))
		local high2Count = #getHighlightedText2(text)/3
		local highlightData3 = getHighlightedText3(text)
		local high3Count = #getHighlightedText3(text)/3
		
		if #highlightData2 > 0 then
		  ...
		  labelHighlight:setColor("green")

		  -- Remove the curly braces
		  
		  for i = 1, #highlightData2 / 3 do
			local dataBlock = { _start = highlightData2[(i-1)*3+1], _end = highlightData2[(i-1)*3+2], words = highlightData2[(i-1)*3+3] }
			text = text:gsub("%{(.-)%}", dataBlock.words, 1)

			-- Recalculate positions as braces are removed
			highlightData2[(i-1)*3+1] = dataBlock._start - ((i-1) * 2)
			highlightData2[(i-1)*3+2] = dataBlock._end - (1 + (i-1) * 2)
		  end
		  ...
		  local tmpText = ""
		  if(#highlightData2 > 0)then
			--tmpText = tmpText .. string.rep(string.char(127), 2)
		  end
		  
		  for i = 1, #highlightData2 / 3 do
			local dataBlock = { _start = highlightData2[(i-1)*3+1], _end = highlightData2[(i-1)*3+2], words = highlightData2[(i-1)*3+3] }
			local lastBlockEnd = (highlightData2[(i-2)*3+2] or 1)
		  ...
		  local finalBlockEnd = (highlightData2[(#highlightData2/3-1)*3+2] or 1)
		  ...
		  --labelHighlight:resizeToText()
		end
		
		--pinfo(text)
		--pinfo(string.find(text, ":%s"))
		local highlightData3 = getHighlightedText3(text:sub(0, string.find(text, ":%s")))
		  		
		if #highlightData3 > 0 then
		  
		  local labelHighlight2 = g_ui.createWidget('ConsolePhantomLabel', label)
		  labelHighlight2:fill('parent')

		  labelHighlight2:setId('consoleLabelHighlight2' .. consoleBuffer:getChildCount())
		  labelHighlight2:setColor("white")

		  -- Remove the curly braces
		  for i = 1, #highlightData3 / 3 do
			local dataBlock = { _start = highlightData3[(i-1)*3+1], _end = highlightData3[(i-1)*3+2], words = highlightData3[(i-1)*3+3] }
			text = text:gsub("%((.-)%)", dataBlock.words, 1)

			-- Recalculate positions as braces are removed
			highlightData3[(i-1)*3+1] = dataBlock._start - ((i-1) * 2)
			highlightData3[(i-1)*3+2] = dataBlock._end - (1 + (i-1) * 2)
		  end
		  label:setText(text)
		  ...
		  for i = 1, #highlightData3 / 3 do
			local dataBlock = { _start = highlightData3[(i-1)*3+1], _end = highlightData3[(i-1)*3+2], words = highlightData3[(i-1)*3+3] }
			local lastBlockEnd = (highlightData3[(i-2)*3+2] or 1)
		  ...
		  -- Fill the highlight label to the same size as default label
		  local finalBlockEnd = (highlightData3[(#highlightData3/3-1)*3+2] or 1)
		  for letter = finalBlockEnd, drawText:len() do
			  local tmpChar = string.byte(drawText:sub(letter, letter))
			  local fillChar = (tmpChar == 10 or tmpChar == 32) and string.char(tmpChar) or string.char(127)
		  ...
		  labelHighlight2:setText(tmpText)
		  labelHighlight2:resizeToText()
		end
  end
  ...
  if speaktype.npcChat then
    ...
    if #highlightData > 0 then
      local labelHighlight3 = g_ui.createWidget('ConsolePhantomLabel', label)
      labelHighlight3:fill('parent')

      labelHighlight3:setId('consoleLabelHighlight3' .. consoleBuffer:getChildCount())
      labelHighlight3:setColor("green")
      ...
      labelHighlight3:setText(tmpText)
    end
  ...
  if labelHighlight2 then
	  labelHighlight2.onMouseRelease = function(self, mousePos, mouseButton)
		processMessageMenu(mousePos, mouseButton, creatureName, text, self, tab)
	  end  
  end
  if labelHighlight3 then
	  labelHighlight3.onMouseRelease = function(self, mousePos, mouseButton)
		processMessageMenu(mousePos, mouseButton, creatureName, text, self, tab)
	  end  
  end
...
function processMessageMenu(mousePos, mouseButton, creatureName, text, label, tab)
  ...
  --[[if mouseButton == MouseLeftButton then
	if text then
	guildBox = displayInfoBox(tr("For Your Information2"), "GUILD" .. text)
	 label:setColor("green")
	  connect(guildBox, { onOk = function(msgbox)
									  guildBox = nil
									end })
	end
  end]]--
...
function getSpecialNameByPrefix(specialName)

	if specialName == "player" then
		specialName = ""
	elseif specialName == "administrator" then
		specialName = "adm"
	elseif specialName == "gamemaster" then
		specialName = "gm"
	elseif specialName == "guardian" or specialName == "banner" then
		specialName = "bn"
	elseif specialName == "consuller" then
		specialName = "cn"
	elseif specialName == "tutor" then
		specialName = "tut"
	elseif specialName == "community manager" then
		specialName = "cm"
	end
...
function applyMessagePrefixies(name, level, message, guild, specialName, guildTaq)
  ...
  if name then
	...
	if (specialName and specialName ~= "") then
		 message = '<{'..string.upper(specialName)..'}>' .. ' ' .. message
	end
  end
  return message
end

function onTalk(name, level, mode, message, channelId, creaturePos, guild, specialName)
  --message = getStringIcon(message)
  ...
  if specialName then
	specialName = getSpecialNameByPrefix(specialName)
  end
  ...
  if (mode == MessageModes.Say or mode == MessageModes.Whisper or mode == MessageModes.Yell or
	...
	--vultur tutaj wiadomosc nad creatura
  ...
  --pinfo("name: "..name.." lvl: "..level.."")
  if (mode == MessageModes.Yell or mode == MessageModes.Say or mode == MessageModes.Whisper or mode == MessageModes.Channel) then
	composedMessage = applyMessagePrefixies(name, level, message, guild, specialName, true)
  else
	composedMessage = applyMessagePrefixies(name, level, message, nil, nil, false)
  end

  if mode == MessageModes.RVRAnswer then
    violationReportTab.locked = false
    addTabText(composedMessage, speaktype, violationReportTab, name, guild, specialName)
  elseif mode == MessageModes.RVRContinue then
    addText(composedMessage, speaktype, name .. '\'...', name, guild, specialName)
  elseif speaktype.private then
    ...
    if channel then
      addText(composedMessage, speaktype, channel, name, guild, specialName)
    else
...
function openSupport()
	g_platform.openUrl("support.priogames.com")
end
...
function online()
  ...
  if g_game.getClientVersion() < 862 then
    g_keyboard.bindKeyDown('Ctrl+R', openSupport)
  end
  -- open last channels
