...
function init()
  ...
  spellPreview	        = spelllistWindow:getChildById('spellPreview')
  ...
  tpValueLabel			= spelllistWindow:getChildById('labelTpValue')
...
function initializeSpelllist()
  for i = 1, #SpelllistSettings[SpelllistProfile].spellOrder do
   ...
   local iconId = tonumber(SpellIcons['intenserecovery'])
   --['intenserecovery'] 
   if not iconId and SpellIcons['intenserecovery'] then
   -- if not iconId and SpellIcons[info.icon] then
      iconId = SpellIcons['intenserecovery'][1]
    end
    ...
    tmpLabel:setImageSource('/images/game/spells/icons/'..spell..'')
    --tmpLabel:setImageClip(Spells.getImageClip(iconId, SpelllistProfile))
    tmpLabel:setImageSize(tosize(35 .. ' ' .. 35))
	tmpLabel:setImageFixedRatio(true)
...
function copyCB()
	g_window.setClipboardText(spelllistWindow:getChildById('labelFormulaValue'):getText());
end